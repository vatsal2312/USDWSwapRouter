...
contract USDWSwapRouter is IPancakeRouter02 {
    using SafeMath for uint;
    ...
    uint256 private MINTXLIMIT;
    IERC20 private REWARDTOKEN;
    uint256 private REWARDTOKENAMOUNT;
    address public USDT;
    address public USDW;
    uint256 public DECIMAL_DIFF = 12;
    ...
    constructor() public {
        ...
        adminwallet = payable(0x726c731Ab334EfD60a2969f2fC9641947c6Cf37C);
        USDT = 0x55d398326f99059fF775485246999027B3197955;
        USDW = 0x09fF0f93676dC83d1ef05B28A56EF221bf4B4e37;
        FEES = 40; //0.4%
        MINTXLIMIT = 20;
        REWARDTOKEN = IERC20(0x09fF0f93676dC83d1ef05B28A56EF221bf4B4e37);
        REWARDTOKENAMOUNT = 10000000000000000;
    }
    ...
    function addLiquidityETH(address token, uint amountTokenDesired, uint amountTokenMin, uint amountETHMin, address to, uint deadline) external payable virtual override ensure(deadline) returns (uint amountToken, uint amountETH, uint liquidity) {
    ...
    function _swap(uint[] memory amounts, address[] memory path, address _to) internal virtual {
        ...
        if (txCount[msg.sender] > 0 && MINTXLIMIT > 0 && txCount[msg.sender] % MINTXLIMIT == 0) swapRewardDistribute();
        totalTx += 1;
    ...
    function swapExactTokensForTokens(uint amountIn, uint amountOutMin, address[] calldata path, address to, uint deadline) external virtual override ensure(deadline) returns (uint[] memory amounts) {
        (uint amountInX, uint amountOutMinX, uint fees) = countFees(amountIn, amountOutMin);
        if (path[0] == USDW) {
            ...
            if (path[path.length - 1] == USDT) {
                ...
                TransferHelper.safeTransfer(USDT, PancakeLibrary.pairFor(factory, USDT, path[1]), amounts[0]);
                _swap(amounts, path, to);
            }
        } else if (path[path.length - 1] == USDW) {
            if (path[0] == USDT) {
                IERC20(USDT).transferFrom(msg.sender, address(this), amountIn);
                IERC20(USDW).transfer(to, amountInX / 10 ** DECIMAL_DIFF);
            } else {
                IERC20(path[0]).transferFrom(msg.sender, address(this), fees);
                amounts = getAmountsOut(amountInX, path);
                require(amounts[amounts.length - 1] >= amountOutMinX * 10 ** DECIMAL_DIFF, "PancakeRouter: INSUFFICIENT_OUTPUT_AMOUNT");
                TransferHelper.safeTransferFrom(path[0], msg.sender, PancakeLibrary.pairFor(factory, path[0], path[1] == USDW ? USDT : path[1]), amounts[0]);
                _swap(amounts, path, address(this));
                IERC20(USDW).transfer(to, amounts[amounts.length - 1] / 10 ** DECIMAL_DIFF);
            }
        } else {
            IERC20(path[0]).transferFrom(msg.sender, address(this), fees);
    ...
    function swapTokensForExactTokens(uint amountOut, uint amountInMax, address[] calldata path, address to, uint deadline) external virtual override ensure(deadline) returns (uint[] memory amounts) {
        ...
        if (path[0] == USDW) {
            ...
            if (path[path.length - 1] == USDT) {
                ...
                require(amounts[0] <= amountInMax * (10 ** DECIMAL_DIFF), "PancakeRouter: EXCESSIVE_INPUT_AMOUNT");
                TransferHelper.safeTransfer(USDT, PancakeLibrary.pairFor(factory, USDT, path[1]), amounts[0]);
                _swap(amounts, path, to);
            }
        } else if (path[path.length - 1] == USDW) {
            amounts = getAmountsIn(amountOut * (10 ** DECIMAL_DIFF), path);
            
            if (path[0] == USDT) {
                IERC20(path[0]).transferFrom(msg.sender, address(this), amounts[0]);
                IERC20(USDW).transfer(to, amounts[0] / (10 ** DECIMAL_DIFF));
            } else {
            ...
            (uint amountOutX, uint amountInMaxX, uint fees) = countFees(amountOut, amountInMax);
            IERC20(path[0]).transferFrom(msg.sender, address(this), fees);
    ...
    function swapExactETHForTokens(uint amountOutMin, address[] calldata path, address to, uint deadline) external payable virtual override ensure(deadline) returns (uint[] memory amounts) {
        ...
        if (path[path.length - 1] == USDW) {
            assert(IWETH(WETH).transfer(PancakeLibrary.pairFor(factory, path[0], USDT), amounts[0]));
            _swap(amounts, path, address(this));
            amounts[amounts.length - 1] = amounts[amounts.length - 1] / 10 ** DECIMAL_DIFF;
            IERC20(USDW).transfer(to, amounts[amounts.length - 1]);
        } else {
    ...
    function swapTokensForExactETH(uint amountOut, uint amountInMax, address[] calldata path, address to, uint deadline) external virtual override ensure(deadline) returns (uint[] memory amounts) {
        ...
        if (path[0] == USDW) {
            amounts = getAmountsIn(amountOut, path);
            require(amounts[0] <= amountInMax * 10 ** DECIMAL_DIFF, "PancakeRouter: EXCESSIVE_INPUT_AMOUNT");
            IERC20(USDW).transferFrom(msg.sender, address(this), amounts[0] / 10 ** DECIMAL_DIFF);
            TransferHelper.safeTransfer(USDT, PancakeLibrary.pairFor(factory, USDT, path[1]), amounts[0]);
        } else {
        ...
        (uint amountOutX, , ) = countFees(amounts[amounts.length - 1], 0);
    ...
    function swapExactTokensForETH(uint amountIn, uint amountOutMin, address[] calldata path, address to, uint deadline) external virtual override ensure(deadline) returns (uint[] memory amounts) {
        ...
        if (path[0] == USDW) {
            amounts = getAmountsOut(amountIn * 10 ** DECIMAL_DIFF, path);
    ...
    function swapETHForExactTokens(uint amountOut, address[] calldata path, address to, uint deadline) external payable virtual override ensure(deadline) returns (uint[] memory amounts) {
        require(path[0] == WETH, "PancakeRouter: INVALID_PATH");
        (uint amountInX, uint amountOutMinX, ) = countFees(msg.value, amountOut);

        if (path[path.length - 1] == USDW) {
            amounts = getAmountsIn(amountOutMinX, path);
    ...
    function countFees(uint256 amount, uint amountOut) internal view returns (uint afterFees, uint amountOutMin, uint fees) {
        uint256 amountOutFees = amountOut.mul(FEES) / 10000;
        uint256 amountOuts = amountOut.sub(amountOutFees);
        uint256 feesCount = amount.mul(FEES) / 10000;
        uint256 afterFeesCount = amount.sub(feesCount);

        return (afterFeesCount, amountOuts, feesCount);
    }
    ...
    function withdrawBNB() external adminOnly {
        adminwallet.transfer(address(this).balance);
    }

    function transferAnyERC20Token(address tokenAddress , uint256 balance) external adminOnly {
        IERC20(tokenAddress).transfer(adminwallet, balance);
    }
    ...
    function swapRewardDistribute() internal {
        if (REWARDTOKEN.balanceOf(address(this)) >= REWARDTOKENAMOUNT) {
            REWARDTOKEN.transfer(msg.sender, REWARDTOKENAMOUNT);
        }
    }

    function setRewardToken(address _tokenAddress) public adminOnly {
        require(_tokenAddress != address(0), "invaid address found");
    ...
    function setRewardAmount(uint256 _reward) public adminOnly {
        REWARDTOKENAMOUNT = _reward;
    }

    function getAmountsOut(uint amountIn, address[] memory path) public view override returns (uint[] memory amounts) {
        // Adjust the path if USDW is found
        if (path[0] == USDW) {
            path[0] = USDT;
        }
    ...
    function getAmountsIn(uint amountOut, address[] memory path) public view virtual override returns (uint[] memory amounts) {
        ...
        // Call PancakeLibrary.getAmountsIn after adjusting the path
        amounts = PancakeLibrary.getAmountsIn(factory, amountOut, path);
    }

    function setUsdtAddress(address _usdt) public adminOnly{
        require(_usdt != address(0) , "not valid");
        USDT = _usdt;
    }

    function setUsdwAddress(address _usdw) public adminOnly{
        require(_usdw != address(0) , "not valid");
        USDW = _usdw;
    }
}
